...
//				this.content.removeClass('visibility-hidden');
	...
	parseXML: function() {
		...
		var button = new Element('div').addClass("buttonContainer").setStyle('width', button_width);
		...
		if(this.options[Field.PROPERTY_TOOLTIP]) {
	    	...
	    	if(this.xml.getAttribute(Field.PROPERTY_VALUE))
	    		button.set('title', this.xml.getAttribute(Field.PROPERTY_VALUE));
	    	else if(this.options[Field.PROPERTY_VALUE])
	    		button.set('title', this.options[Field.PROPERTY_VALUE]);
	    }
		...
		if(real_btn) {
			real_btn.setStyles({
				'min-height': Generic.getHiddenHeight(real_btn),
				height: 'auto',
				width: '100%'
			});
	...
	parseXMLforGrid: function(td_container, grid_index, isGridReadonly) {
		...
		var button = new Element('div').addClass("gridButtonContainer").setStyles({'width': button_width, 'margin': 'auto'});
