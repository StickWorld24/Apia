...
function initPage() {
	...
	if (RELOAD_TAB) {
		var tabContainer = getTabContainerController();
		if (tabContainer) {
			var activeTab = tabContainer.activeTab;
			activeTab.addEvent('onFocusTab', function(e) {
				if (e) e.stop();

				var acqIframe = activeTab.content.contentDocument.getElementById('iframeAcquired');
				var readyIframe = activeTab.content.contentDocument.getElementById('iframeReady');
				var btnRefresh = acqIframe.contentDocument.getElementById('navRefresh');
				if (btnRefresh)
					btnRefresh.fireEvent("click");
				
				btnRefresh = readyIframe.contentDocument.getElementById('navRefresh');
				if (btnRefresh)
					btnRefresh.fireEvent('click');
			});
		}	
	}
//	$('bodyDiv').onTabFocus = fncOnTabFocus;
}
