...
//		console.log('fooooooooooooooorm');
//		console.log(form);
...
//			}
	...
	if(btnBackToList) {
		btnBackToList.addEvent('click', function(e) {
			...
			if(CLOSE_ON_RETURN){
				getTabContainerController().removeTabByID(TAB_ID);
				return;
			}
	...
	if (btnCloseMdl){
		btnCloseMdl.addEvent('click', function(e) {
			if(window.frameElement && $(window.frameElement).hasClass('modal-content')) {
				$(window.frameElement).fireEvent('closeModal');
				//llamar al server para liberar memoria
				
				var request = new Request({
					method: 'post',
					url: CONTEXT + URL_REQUEST_AJAX + '?action=admEntityClose&isAjax=true&' + TAB_ID_REQUEST,
					onRequest: function() { },
	...
	// start drag and drop functionalities
	initDroppedDocsFunctions('apia.execution.EntInstanceListAction.run?', 'E', 'dropAreaE', true, true, null, 'frmOut=true',
		{
...
function processModalEntInstImageConfirm(image) {
	$('entInstImgPicker').setStyle('background-image', 'url(' + image.path + ')');
	var src = $('txtEntInstImg');
	src.value = image.id;
...
function onfinishgoToUrl(){
	window.parent.location = ON_FINISH_URL;
}