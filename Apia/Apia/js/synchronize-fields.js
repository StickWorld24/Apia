...
SynchronizeFields.syncAJAX = function(container, value, retries) {
	...
	if( (element.options[Field.PROPERTY_DISABLED] && element.options[Field.PROPERTY_DISABLED]=="T") || (element.options[Field.PROPERTY_INPUT_AS_TEXT] && element.options[Field.PROPERTY_INPUT_AS_TEXT] == "T") || element.form.readOnly) {
	...
	for(var i1 = 0; i1 < response.childNodes.length; i1++) {
		...
		if(form.tagName == "z") {
			...
			if($(fldId_focus)){
				if($(fldId_focus).getElements("input") && $(fldId_focus).getElements("input").length >0){
					if($(fldId_focus).getElements("input").length==1){
						$(fldId_focus).getElements("input")[0].focus();
					} else if($(fldId_focus).getElements("input").length==2){//campo fecha que en onblur dispara el onchange
						
						if($(fldId_focus).parentElement && $(fldId_focus).parentElement.nextSibling){
							if($(fldId_focus).parentElement.nextSibling.getElementsByTagName('input').length>0 ){
								$(fldId_focus).parentElement.nextSibling.getElementsByTagName('input')[0].focus();
							}
							if($(fldId_focus).parentElement.nextSibling.getElementsByTagName('button').length>0 ){
								$(fldId_focus).parentElement.nextSibling.getElementsByTagName('button')[0].focus();
							}	
						}
		...
		for(i = 0; i < form.childNodes.length; i++) {
			if(frm_src.fields) {
				...
				var trads = form.childNodes[i].getAttribute("trad");
				var fld_src = null;
				...
				if(fld_src.setTranslations){
					fld_src.setTranslations(JSON.decode(trads))
				}
