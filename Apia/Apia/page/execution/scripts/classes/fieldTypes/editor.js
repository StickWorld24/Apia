var Editor = new Class({
	...
	apijs_setFieldValue: function(value) {
		...
		if(!this.options[Field.PROPERTY_READONLY]){
			tinymce.get('area_' + this.content.id).setContent(strValue + "");	
			this.content.getElement('textarea').set('value', strValue + "");
		} else {
			$("div_" + this.content.id).set('html',strValue);
		}
	},
	...
	parseXML: function() {
		...
		*/
		...
		if(this.options[Field.PROPERTY_READONLY] || this.form.readOnly) {
			...
			editor.set('data-position',inputsCount++);
