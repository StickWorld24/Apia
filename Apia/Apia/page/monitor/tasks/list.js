...
function setFilter(back){
	...
	*/
	callNavigateFilterTasks(null, "txtTskTitle=" + $('cmbTskTitFilter').value +
			"&txtProTitle=" + ($('cmbProcTitFilter').value != "" ? $('cmbProcTitFilter').options[cmbProcTitFilter.selectedIndex].textContent : "") +
			"&filProName=" + $('monInstProNroRegFilter').value +
			"&cmbSta=" + $('statusFilter').value +
			"&txtProPool=" + $('proPoolFilter').value +
			"&txtProInstUser=" + $('monInstProCreUsuFilter').value +
...
function validateDates(obj){
	...
	} else if (obj.id.startsWith('txtRea')) {
		var dteStart = $('txtReaSta');
		var dteEnd = $('txtReaEnd');
	} else if (obj.id.startsWith('txtEnd')) {
		var dteStart = $('txtEndSta');
		var dteEnd = $('txtEndEnd');
