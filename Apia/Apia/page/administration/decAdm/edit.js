function initPage(){
	...
	$('addGenPool').addEvent("click", function(e) {
		e.stop();
		//setear variables de configuracion del modal de grupos
		POOLMODAL_SHOWAUTOGENERATED = false;
		POOLMODAL_SHOWNOTAUTOGENERATED = true;
		...
		//abrir modal
		showPoolsModal(processGenPoolsModalReturn);
	});
...
function loadEnvs(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserEnvironments&isAjax=true' + TAB_ID_REQUEST,
...
function loadPools(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserPools&disableHie=true&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function loadProfiles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserProfiles&disableHie=true&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function loadOrgRoles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserOrgRoles&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
...
function loadStyles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserStyles&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { processXMLUsrStyles(resXml); sp.hide(true); }
	}).send();
}

function loadGenPools(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX+ '?action=getUserGenPools&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLUsrGenPools(resXml);
			initAdminModalHandlerOnChangeHighlight($('genPoolsContainter'));
			sp.hide(true); }
	}).send();
}
...
function processXMLUsrGenPools(ajaxCallXml){
	if (ajaxCallXml != null) {
		var envs = ajaxCallXml.getElementsByTagName("pools");
		if (envs != null && envs.length > 0 && envs.item(0) != null) {
			envs = envs.item(0).getElementsByTagName("pool");
			for(var i = 0; i < envs.length; i++) {
				...
				var id 		= env.getAttribute("id");
				var addRemove = true;
				addActionElementAdmin($('genPoolsContainter'), text, id, "false", addRemove, "hidGenPool");
			}
		}
	}
}
...
function processGenPoolsModalReturn(ret) {
	ret.each(function(e) {
		var text = e.getRowContent()[0];
		addActionElement($('genPoolsContainter'),text,e.getRowId(),"hidGenPool");
	})
}
