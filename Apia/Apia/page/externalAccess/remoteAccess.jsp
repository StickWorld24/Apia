...
if("SPLASH".equals(action) && request.getParameter("token")==null) {
	if (versionNumber >= 3 || versionNumber == -1) {
		//pageContext.forward("../../apia.security.LoginAction.run?action=redirectToSplash");
		response.sendRedirect("../../apia.security.LoginAction.run?action=redirectToSplash&"+request.getQueryString());
	} else {
	...
	if (error==MyUtilClass.ERROR_NO) { //si apia permite acceso remoto o es una version anterior a la 3.0.0.1
		...
		String userPass = (encryptedPass!=null && !"".equals(encryptedPass))?ExternalService.decryptApia(userPwd):"";
			
		boolean loginOk = false;
		if(request.getParameter("token")!=null){
			//obtener url del otro apia...
			String remoteURL = Configuration.TRUSTED_APIA_URL + "/page/checkToken.jsp?user=" + userLogin + "&token=" + request.getParameter("token");
			String data = HTTPUtils.getURLData(remoteURL);
			if(data.equals("true")) {
				loginOk = true;
				System.out.println("loginOk:true");
			}	
		} else {	
			loginOk = utilClass.checkLogin(userLogin, userPass);
