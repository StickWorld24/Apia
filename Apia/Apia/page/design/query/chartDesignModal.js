...
function chtTypeChange(cleanComboSubType){
	...
	if (chtType==BAR_VER_TYPE_ID){//BARRAS VERTICALES
		...
		//desbloqueamos la customizacion de colores
		unLockSerieColorCustomization();
		...
		//bloqueamos la customizacion de colores
		lockSerieColorCustomization();
...
function showXYProps(show) {
	if (show) {
		...
		$('divMorePrps').style.visibility = 'hidden';
	}
}
...
function lockSerieColorCustomization(){
	var id=$("chtColX").options[$("chtColX").selectedIndex].text;
	var rows = $("bodySeries").getElements('tr');
	...
	for(var i = 0; i < rows.length; i++) {
		...
		tds[1].getElement('img').setStyle('visibility', 'hidden');
		tds[1].getElements('input')[0].setStyle('visibility', 'hidden');
		tds[1].getElements('input')[1].setStyle('visibility', 'hidden');
	}
	
} 

function unLockSerieColorCustomization(){
	var id=$("chtColX").options[$("chtColX").selectedIndex].text;
	var rows = $("bodySeries").getElements('tr');
	...
	for(var i = 0; i < rows.length; i++) {
		...
		var tds = rows[i].getElements('td');
		tds[1].getElement('img').setStyle('visibility', '');
		tds[1].getElements('input')[0].setStyle('visibility', '');
		tds[1].getElements('input')[1].setStyle('visibility', '');
	}
