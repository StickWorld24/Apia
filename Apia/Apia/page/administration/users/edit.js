function initPage(){
	...
	if($('environmentsContainter')){
		//mostrar la caja "todos lso ambientes" si:
		//-el usuario es para todos los ambientes
		//-se esta creando un usuario global
		var eleAllEnvs = addActionElementAdmin($('environmentsContainter'), LBL_ALL_ENVS,"allEnvs","true",false,null);
		if(isAllEnvs || (isGlobal && isCreate)){
			eleAllEnvs.setStyle("display",'');
		} else {
			eleAllEnvs.setStyle("display",'none');
		}
	}
...
function resetUsrImage() {
	var btnResetImage = $('btnResetImage');
	var imgUsr = $('imgUsrImage');
	if(btnResetImage) {
		if (imgUsr) {
			imgUsr.src = CONTEXT + "/getImageServlet.run?height=24&width=24";
			imgUsr.fireEvent('change');
			
			var request = new Request({
				method: 'post',
				url: CONTEXT + '/apia.administration.UsersAction.run?action=deleteUsrImageFromEdit&isAjax=true&' + TAB_ID_REQUEST
//				onRequest: function() { },
