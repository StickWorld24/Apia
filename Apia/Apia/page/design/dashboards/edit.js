...
function initPage(){
	...
	$('flagShowFunc').set('disabled',!$('flagCreateFunc').get('checked'));
	
	$('flagCreateFunc').addEvent('change',function(e){
		e.stop();
		$('flagShowFunc').set('disabled',!this.get('checked'));
		if(!this.get('checked')){
			$('flagShowFunc').set('checked',false);
		}
	...
	if (parameters != null && parameters.length > 0 && parameters.item(0) != null) {
		...
		for (var i = 0; i < parameters.length; i++){
			...
			if (xmlParam.getAttribute("viewType") == PNL_PARAM_VIEW_TYPE_COMBOBOX || 
					...
					xmlParam.getAttribute("viewType") == PNL_PARAM_VIEW_TYPE_MDL_SCH  ||
					...
					xmlParam.getAttribute("viewType") == PNL_PARAM_VIEW_TYPE_MDL_FLD){
