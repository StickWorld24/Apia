<%@page import="biz.statum.apia.web.bean.BasicBeanStatic"%><div class="tabContent"><div class="fieldGroup"><div class="title"><system:label show="text" label="sbtEjeDatPro" /></div><div class="field required"><%if (proInstVo.getProInstNameNum() != null) {
				...
				%><span class="monitor-lbl" title="<system:label show="tooltip" label="lblEjeIdePro" />"><system:label show="text" label="lblEjeIdePro" />:<%if (ProcessVo.IDENTIFIER_NUM_WRITE.equals(proVo.getProIdeNum())){%>* <%}%></span><%
				...
				if (ProcessVo.IDENTIFIER_NUM_AUTO.equals(proVo.getProIdeNum())) {
					...
					%><input title="<system:label show="tooltip" label="lblEjeIdePro" />" size=9 maxlength=9 name="txtProNum" id="txtProNum" value="<%=BeanUtils.fmtInt(proInstVo.getProInstNameNum())%>" <%if (ProcessVo.IDENTIFIER_NUM_WRITE.equals(proVo.getProIdeNum())){%> class="validate['required']" <%}%>><%
				}
