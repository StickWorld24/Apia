...
var FormCheck = new Class({
	...
	options : {
		...
		submit : true,						// false : just validate the form
											// and do nothing else. Use
											// onValidateSuccess event to
											// execute some code
		submitAction: false,				// Action page used to submit the
											// form data to.
		submitMethod: false,				// Method used to submit the form,
											// valid options : 'post' or 'get'

		trimValue : false,					// trim (remove whitespaces before
											// and after) the value
		...
		onAjaxRequest : $empty,				// Function to fire when the Request
											// event starts
		onAjaxComplete : $empty,			// Function to fire when the Request
											// is complete, before and
											// regardless of Success or Failure
		onAjaxSuccess : $empty,				// Function to fire when the Request
											// receives . Args: response [the
											// request response] - see Mootools
											// docs for Request.onSuccess
		onAjaxFailure : $empty,				// Function to fire if the Request
											// fails

		onSubmit		  : $empty,			// Function to fire when user submit
											// the form
		...
		regexp : {
			...
			phone : /^\+{0,1}[0-9 \(\)\.\-]+$/, // alternate regex : /^[\d\s
												// ().-]+$/,/^((\+\d{1,3}(-|
												// )?\(?\d\)?(-|
												// )?\d{1,5})|(\(?\d{2,6}\)?))(-|
												// )?(\d{3,4})(-| )?(\d{4})(( x|
	...
	/*
	 * Function: register Allows you to declare afterward new fields to the
	 * formcheck, to check dynamically loaded fields for example. By default it
	 * will be the last element to be validated as it's added after others
	 * inputs, but you can define a position with second parameter.
	 * 
	 * Example: (code) <script type="text/javascript">
	 * window.addEvent('domready', function() { formcheck = new
	 * FormCheck('form_id'); }); // ...some code...
	 * 
	 * var newField = new Element('input', { class : "validate['required']",
	 * name : "new-field" }).inject('form_id'); formcheck.register(newField, 3);
	 * 
	 * new Element('input', { class : "validate['required']", name :
	 * "another-field", id : "another-field" }).inject('form_id');
	 * formcheck.register($('another-field')); </script> (end code)
	 * 
	 * See also: <FormCheck::dispose>
	 */
	...
	/*
	 * Function: dispose Allows you to remove a declared field from formCheck
	 * 
	 * Example: (code) <script type="text/javascript">
	 * window.addEvent('domready', function() { formcheck = new
	 * FormCheck('form_id'); }); // ...some code...
	...
	manageError : function(el, method) {
		...
		if ((!isValid && el.validation.contains('required')) || ((el.value || el.value == "" || el.get('forceReq') == 'true') && !isValid) || (!isValid && el.validation.contains('~validLabel'))) {
			if (this.options.display.listErrorsAtTop) this.listErrorsAtTop(el);
	...
	// el_target.focus();
								...
								var new_scroll_y = el_target.getCoordinates().top - 70; // Por
																						// el
																						// mensaje
																						// que
																						// aparece
	...
	/*
	 * Function: validateWords Private method
	 * 
	 ...
	 */
	 isFormValid: function() {
		 //debugger;
		 	if(this.form.get('id') == 'frmData'){
		 		this.options.display.listErrorsAtTop = true;
			 	// remove grid css error class
			 	$$('.grid-error-class').each(function(element) {(element.removeClass('grid-error-class'))});
			 	
			 	// enumeate div.wasTd or td tag with an ordernumber
				// this number will be used to sort error list at top;
				var iter = 0; 
				$$('div.wasTd, td').each(function(item){
					return item.set('data-ordernumber',++iter);
				});
				
			 	// Initilize list of errors
			 	indexError = 0;
				if($('errorlist') != null){
					$('errorlist').getChildren().each(function(errorItem){ errorItem.dispose()});
					$('errorlist').dispose();
				}
		 	}else{
		 		this.options.display.listErrorsAtTop = false;
		 		// remove errors style
		 		$$(this.options.fieldErrorClass).each(function(e){e.removeClass(this.options.fieldErrorClass)});
			...
			/*
			 * if (this.form.isValid) { this.fireEvent('validateSuccess');
			 * return true; } else { this.focusOnError(this.firstError);
			 * this.fireEvent('validateFailure'); return false; }
			 */
			if(this.form.get('id') == 'frmData'){
				sortElementAtTopListError();
				if($('errorlist')!= null){
					var errorsArray = $('errorlist')
					.getChildren()
					.filter(function(item){
						return item.get('data-ordernumber')!= null
					})
					.map(function(errorItem){
						return $$('div.wasTd[data-ordernumber='+errorItem.get('data-ordernumber')+'], td[data-ordernumber='+errorItem.get('data-ordernumber')+']')[0]
						})
					//en el map sustituyo el error del indice de errores, 
					//por su correspondiente en el formulario
					.filter(function(item) {
						return item.getChildren().filter(function(i){
							return i.hasClass('gridMinWidth')
								}).length >0
								});
					 errorsArray.each(function(error){ error.addClass('grid-error-class')});
...
//debugger;
			if(!this.form.element) {
				 this.form.element = new Element('div', {'id' : 'errorlist', 'class' : this.options.errorClass, name : "portalFormTemplate"}).injectTop(this.form);
			}
		...
		if ($type(obj) == 'collection') {
			...
			if ((obj.validation.contains('required') && obj.errors.length > 0) || (obj.errors.length > 0 && obj.value && obj.validation.contains('required') == false)) {
				//debugger;
				// get the td parent of obj;
				var tdParent = obj.getParent();
				while (!tdParent.hasClass('wasTd') && tdParent.get('tag')!='td'){
					tdParent = tdParent.getParent();
				}; 
...
//					if(obj.get("data-attLabel")==null){
						let objParent = null;
						if(obj.type == 'radio') {
							objParent = obj.getParent().getParent().getParent();
						} else {
							objParent = obj.getParent();
						}
						let spanBrother = objParent.getElementsByTagName('span')[0];
						if(typeof spanBrother !== 'undefined'){
							let label = spanBrother.innerHTML;
							if(label.endsWith(":")) {
								label = label.substring(0,(label.length-1));
							}
							obj.setAttribute("data-attLabel", label);
							new Element('div').set('class','errorBlock')
							.set('data-ordernumber', tdParent.get("data-ordernumber"))
							.set('data-errorNumber', indexError)
							.set('onClick', 'goToError(this); return false;')
							.set('html',"<div class='leftErrorSpan'>" + indexError + ". " + obj.get("data-attLabel")  
									+ ": </div> <div class='rightErrorSpan'><a href=#>" + error + "</a> </div>")
							.injectInside(this.form.element).scrollIntoView( false );
						} else{
							// form inputs
							var id = null;
							if (tdParent.get('id')==null){
								id = tdParent.getChildren()[0].get('id');
							}else {
								id = tdParent;
							}
							
							var title;
							(obj.get("title") != null) ? title=obj.get("title") : title=" ";
							
							new Element('div').set('class','errorBlock')
							.set('data-ordernumber', tdParent.get("data-ordernumber"))
							.set('data-errorNumber', indexError)
							.set('onClick', 'goToError(this); return false;')
							.set('html',"<div class='leftErrorSpan'>" + indexError + ". " + title  
									+ ": </div> <div class='rightErrorSpan'><a href=#>" + error + "</a> </div>")
							.injectInside(this.form.element).scrollIntoView( false );
...
//			}
	...
	addError : function(obj) {
		...
		if(!obj.element && this.options.display.indicateErrors != 0) {
			if (this.options.display.errorsLocation == 1) {
				...
				if(objWhere.getParent().get('tag') == 'label'){
					...
					if(objWhere.getParent().get('tag') == 'div' && objWhere.getParent().hasClass('verticalElementsContainer')){
						objWhere = objWhere.getParent();
					}
				}
				if (objWhere.hasClass('datePicker')){
					obj.element.inject(objWhere.getParent());
				}else{
		...
		if (this.options.display.addClassErrorToField){
			...
			if(obj.type=='radio'){
				obj.getParent().getParent().getParent().getParent().addClass(this.options.fieldErrorClass);
			}
...
//debugger;
		...
		var objParent = obj.getParent();
		if (objParent != null){
			//remove error style (red background)
			while (objParent != null && !objParent.hasClass('wasTr') && (objParent.get('tag') != "tr")){
				if (objParent.hasClass('exec_field')){
					objParent.removeClass(this.options.fieldErrorClass);
				}
				if (objParent.hasClass('grid-error-class')){
					objParent.removeClass('grid-error-class');
				}
				objParent = objParent.getParent();
			}
		}
	...
	/*
	 * Function: reinitialize Reinitialize form before submit check. You can use
	 * this also to remove all tips from a form, passing the argument "forced" (
	...
	onSubmit: function(event) {
		...
		if (this.form.isValid) {
			...
			// moved above to allow optional settings to this.form.submit and
			// submitByAjax to be triggered by this option
			return (this.options.submitByAjax)? this.submitByAjax():this.options.submit;
			// if this.options.submit is false it can still rely on
