function initPage(){
	...
	['orderByName', 'orderByDate'].each(setAdmOrder);
	['orderByName', 'orderByDate'].each(setAdmListTitle);
	['nameFilter'].each(setAdmFilters);
	['dateFilter'].each(setDateFilters);
	
	$('clearFilters').addEvent("click", function(e) {
		e.stop();
		['nameFilter'].each(clearFilter);
		['dateFilter'].each(clearFilterDate);
	...
	btnSync = $("btnSync");
	if (btnSync) {
		btnSync.addEvent('click', function(e){
			e.stop();		
			hideMessage();
			var request = new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX + '?action=synchronizeStyles&isAjax=true' + TAB_ID_REQUEST,
				onRequest: function() { SYS_PANELS.showLoading(); },
				onComplete: function(resText, resXml) { modalProcessXml(resXml); }
			}).send();		
		});
	...
	$('btnReload').addEvent("click", function(e) {
		e.stop();
		var request = new Request({
			method: 'post',
			url: CONTEXT + URL_REQUEST_AJAX + '?action=reloadStyles&isAjax=true' + TAB_ID_REQUEST,
			onRequest: function() { SYS_PANELS.showLoading(); },
			onComplete: function(resText, resXml) { $('navRefresh').fireEvent("click"); SYS_PANELS.closeAll(); }
		}).send();
	});
