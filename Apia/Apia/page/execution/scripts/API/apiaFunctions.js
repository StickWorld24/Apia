...
ApiaFunctions.setStepTitle = function(stepNumber, stepTitle){
	stepId = 'stepNum-' + stepNumber;
	var element = $(stepId);
	element.set('title', stepTitle);	
	if (element.tagName.toUpperCase() === 'DIV'){ // PORTAL FORM TEMPLATE
		var html = "<div class='stepContainer'><div class='stepIdContainer'>"+stepNumber+"</div><div class='stepIdComplete'></div><div class='stepTitleContainer'>"+stepTitle+"</div></div>";
		$(stepId).set('html', html);		
	}
}
...
//ApiaFunctions.getCurrentProcessName = function() {
...
ApiaFunctions.admEntity = function(entName, entNum, parameters, width, height, onShowEvent, onCloseEvent) {
	...
	return ModalController.openWinModal(url, width, height, null, null, true, true, true, null, onShowEvent, onCloseEvent);
}
