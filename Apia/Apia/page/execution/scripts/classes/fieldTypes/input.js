...
var Input = new Class({
	...
	setDefaultOptions: function() {
		...
		this.options[Field.PROPERTY_MODAL_DISABLED]		= null;
	},
	
	booleanOptions: [Field.PROPERTY_INPUT_AS_TEXT, Field.PROPERTY_REQUIRED, Field.PROPERTY_DISABLED, Field.PROPERTY_MODAL_DISABLED, Field.PROPERTY_READONLY, Field.PROPERTY_VISIBILITY_HIDDEN, Field.PROPERTY_STORE_MODAL_QUERY_RESULT, Field.PROPERTY_TRANSLATION_REQUIRED, Field.PROPERTY_SHOW_TOOLTIP_AS_HELP],
	...
	apijs_setFieldValue: function(value) {
		...
		if(!this.qryId || this.qry_show_value == undefined) {
			...
			this.checkTranslationIconVisibility(this.content.getElement('input'));
			this.refreshTranslationMenu();
		}
	},
	...
	apijs_setProperty: function(prp_name, prp_value) {
		...
		if(prp_name == Field.PROPERTY_NAME) {
			...
			if((prp_boolean_value == true || "true" == prp_value ) && this.options[Field.PROPERTY_READONLY] == false) {
				this.content.getElement('input').set('readonly', 'readonly').addClass('readonly');
				this.options[Field.PROPERTY_READONLY] = true;
				if(this.options.valueType == "D") {
					this.content.getElements('input')[1].set('readonly', 'readonly').addClass('readonly');
				...
				this.content.getElement('input').set('readonly', false).removeClass('readonly');
				this.options[Field.PROPERTY_READONLY] = false;
				if(this.options.valueType == "D") {
					//Se le agrega la mascara de las fechas
					if(this.xml.getAttribute('mask')) {
						this.content.getElements('input')[1].set('readonly', false).removeClass('readonly');
...
//			alert(11);
//			processRegExp(this.content.getElement('input'));
//		});
//		
		input.addEvent('change', function(e) {
			this.processRegExp(this.content.getElement('input'));
		}.bind(this));	;	
		...
		if(this.options[Field.PROPERTY_TOOLTIP]){
            ...
            label.set('title', this.xml.getAttribute("attLabel"));
            input.set('title', this.xml.getAttribute("attLabel"));
		}
		...
		if(this.qryId) {
			...
			mdl.addEvent('click', function(e) {
				...
				if(!this.options[Field.PROPERTY_DISABLED] && !this.options[Field.PROPERTY_MODAL_DISABLED]) {
	...
	parseXMLforGrid: function(td_container, grid_index, isGridReadonly) {
		...
		if(this.form.readOnly || isGridReadonly || this.options[Field.PROPERTY_INPUT_AS_TEXT]) {
			...
			if(this.hasBinding){
				input.set('value', this.row_xml.getAttribute("qry_show_value"));
			}
		...
		if(this.options[Field.PROPERTY_REQUIRED]) {		
			...
			if(this.form.options[Form.PROPERTY_FORM_HIDDEN] == "true") {
				input.set('data-disabledCheck', 'true');
			}
		...
		if(this.qryId) {
			...
			if(this.options[Field.PROPERTY_STORE_MODAL_QUERY_RESULT]) {
				...
				//input.set('value', this.qry_show_value);
				input.set('value',new Element('div').set('html',this.qry_show_value).innerText);
			}
	...
	showFormListener: function() {
		var input = this.content.getElement('input');
		if(input)
			input.erase('data-disabledCheck');
	},
			...
			onSuccess: function(responseText, responseXML) {
		    	...
		    	if(responseXML && responseXML.childNodes && responseXML.childNodes.length) {
		    		...
		    		if(response.getAttribute('success')) {
		    			...
		    			var leftField = new Element('div.field.exec_field').inject(data.content);
		    			...
		    			var rightField = new Element('div.field.exec_field').inject(data.content);
		    			...
		    			ModalController.openContentModal(data, 700, 240);
		    		}
		    	}
			}
		}).send();
		...
		if(this.xml.getAttribute('regExp')) {
			...
			if(!result) {
				if(this.options[Field.PROPERTY_REGEXP_MESSAGE])
					showMessage(this.options[Field.PROPERTY_REGEXP_MESSAGE],null,null, function(){element.focus();});
				else
					showMessage(MSG_INVALID_REG_EXP,null,null, function(){element.focus();});
