<%@include file="../page/includes/startInc.jsp" %><%@page import="com.dogma.bi.BIConstants"%><%@page import="com.dogma.bi.BIEngine"%><%@page import="java.util.Collection"%><jsp:useBean id="cBean" scope="session" class="com.dogma.bean.administration.CubeViewBean"></jsp:useBean><%@page import="java.util.Iterator"%><%@page import="com.dogma.vo.CubeViewVo"%><HTML><head><%@include file="../page/includes/headInclude.jsp" %><script language="javascript">	
	...
	String escapederror = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("error")));
	String escapederrorMsg = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("errorMsg")));
	
	%>
	
	var ERROR = "<%=escapederror%>";
	var ERRORMSG = "<%=escapederrorMsg%>";
	...
	function loadView(viewId, viewName) {
		...
		String escapedmode = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("mode")));
		String escapedschemaId = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("schemaId")));
		String escapedcubeId = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("cubeId")));
		String escapedentityCube = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("entityCube")));
		String escapedprocessCube = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("processCube")));
		String escapedviewId = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("viewId")));
		String escapedenvId = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("envId")));
		String escapedenvApiaGenCube = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("envApiaGenCube")));
		String escapedallEnvApiaGenCube = com.st.util.StringUtil.escapeStr(com.st.util.StringUtil.escapeHTML(request.getParameter("allEnvApiaGenCube")));
		%>
		...
		var url = CONTEXT + '/apia.administration.BIAction.run?action=' + (mode == 'navigator' ? 'openNavigator' : 'openViewer') + '&isAjax=true' + 
			'&forceOpen=true' + 
			'&schemaId=<%=escapedschemaId%>' +
			'&cubeId=<%=escapedcubeId%>' +
			'&entityCube=<%=escapedentityCube%>' +
			'&processCube=<%=escapedprocessCube%>' +
			'&viewId=' + escapedviewId +
			'&envId=<%=escapedenvId%>' +
			'&envApiaGenCube=<%=escapedenvApiaGenCube%>' +
			'&allEnvApiaGenCube=<%=escapedallEnvApiaGenCube%>';
	...
	function genNewDefVw() {
		...
		var url = CONTEXT + '/apia.administration.BIAction.run?action=generateViewAndLoad&mode=<%=escapedmode%>&schemaId=<%=escapedschemaId%>&cubeId=<%=escapedcubeId%>&entityCube=<%=escapedentityCube%>&processCube=<%=escapedprocessCube%>&envId=<%=escapedenvId%>&envApiaGenCube=<%=escapedenvApiaGenCube%>&allEnvApiaGenCube=<%=escapedallEnvApiaGenCube%>';
		var tabContainer = window.parent.getTabContainerController();
