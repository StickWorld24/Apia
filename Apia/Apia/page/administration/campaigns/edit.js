...
function initPage(){
	...
	['timeFrom', 'timeTo'].each(setHourField);
...
//	startHourMssg = FormCheck.makeTooltip($('timeFrom'), "La hora de inicio es incorrecta");
}
...
function addNewTime(evt) {
	...
	if (timeFrom.value != "" && timeTo.value != "") {
		if (checkHourMinute(timeFrom, false) && checkHourMinute(timeTo, false)) {
...
function loadXmlValuesTimes(xml) {
	...
	if (elements != null) {
		for (var i = 0; i < elements.length; i++) {
			...
			if (from.length == 3) from = from.slice(0, 1) + HOUR_SEPARATOR + from.slice(1, from.length);
			else from = from.slice(0, 2) + HOUR_SEPARATOR + from.slice(2, from.length);
			if (from.length == 3) to = to.slice(0, 1) + HOUR_SEPARATOR + to.slice(1, to.length);
			else to = to.slice(0, 2) + HOUR_SEPARATOR + to.slice(2, to.length);
			addNewTimeElement(from, to);
		}
	}
}
...
function checkHourMinute(el, useMessage){
	if (el.value == null || el.value == "") {
		if (useMessage) {
			if (el.id.endsWith('From')) startHourMssg.hide();
			else endHourMssg.hide();
		}
		return true;
	}
	...
	if (valueSplit != null && valueSplit.length == 2){
		...
		if (!hour.test(/([0-1][0-9]|[2][0-3])/) || !min.test(/[0-5][0-9]/)){
			if (useMessage) {
				if (el.id.endsWith('From')) {
					startHourMssg = FormCheck.makeTooltip(el, VALID_HR);
					startHourMssg.show();
				} else {
					endHourMssg = FormCheck.makeTooltip(el, VALID_HR);
					endHourMssg.show();
				}
			}
	        return false;
