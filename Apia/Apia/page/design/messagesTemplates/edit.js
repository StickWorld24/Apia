...
function initPage(){
	...
	updateEditor();
	$('isPlainText').addEvent("click", function(e) {
		updateEditor();
	});
...
//			SYS_PANELS.closeAll();
	...
	$$('li.translations').each(function (li){
		li.addEvent("click",function(e){
			...
			if (!this.fixed){
				this.fixed = true;
				if($('body_'+this.getAttribute("langId")+'_tbl')){
		...
		if (translations != null && translations.length > 0 && translations.item(0) != null) {
			...
			if (languages != null && languages.length > 0 && languages.item(0) != null) {
				for (var i = 0; i < languages.length; i++){
					...
					var bodyMessage = $('isPlainText').checked ?  $('noHtml_body_'+langId) : $('body_'+langId);
					
					// si es texto plano remplazo los caracteres especiales  \n\r por saltos de linea en js '\n'
					body = $('isPlainText').checked ? body.split("\\n\\r").join("\n") : body;
					
					subjectMessage.value = subject;
					bodyMessage.set('value', new String(body));
...
function bodyIsEmpty(){
	var defLang = $('defLang').value;
	var noHtmlEditor = $$('.field.fieldFull.fieldTranslations.required > .textAreaEditor > textArea')[0];
	if ($('isPlainText').checked){
		return noHtmlEditor.value.trim() == '';
	}else {
		return ($('body_'+defLang).value.trim() == '');
	}
}

function checkDefLangValues(){
	var defLang = $('defLang').value;
	if ($('subject_'+defLang).value == '' || bodyIsEmpty()){
		$('lang_'+defLang).fireEvent("click");
...
function showAllFirst(arr1, arr2){
	showAll(arr1);
	hiddenAll(arr2);
}

function showAll(arr){
	arr.each(function(e1) {
		e1.style.display = "block";
	});
}

function hiddenAll(arr){
...
function updateEditor(){
	var htmlWrappers = $$('.editor');
	var noHtmlWrappers = $$('.textAreaEditor');
	if ($('isPlainText').checked){
		showAllFirst(noHtmlWrappers, htmlWrappers);
	}else{
		showAllFirst(htmlWrappers, noHtmlWrappers);
	}
}