...
function addFreeSqlEvents(tr) {
	...
	var inputShowAsInt	= tds[19].getElementsByTagName("INPUT")[0];//INPUT
	...
	inputType.inputShowAsInt	= inputShowAsInt;
...
function executeFreeSqlvent(caller) {
	...
	if (currentValue == COLUMN_DATA_DATE) {	
		...
		if (!caller.inputDefault.get('data-hasDatepicker')){
	...
	caller.inputShowAsInt.style.display = (currentValue == COLUMN_DATA_NUMBER) ? '' : 'none';
...
function generateUser(val0,val1,val2,val3,val4,val5) {
	...
	if (val3 == COLUMN_DATA_DATE) {
		arrayOptions.push({'value':1,'text':lblBoth,'selected':false});	
		arrayOptions.push({'value':0,'text':lblNone,'selected':true});
	...
	//Fila
	arrayCell = new Array();
	aux = {'type':'checkbox',name:'chkShowAsIntFilter',id:'chkShowAsIntFilter','required':false,'checked':false,'display':(val3 == COLUMN_DATA_NUMBER? '':'none'),value:1};
	arrayCell.push(aux);
	arrayTd.push({'display':'',arr:arrayCell,'type':'td'});
