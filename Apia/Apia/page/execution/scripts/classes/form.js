	...
	parseXML: function(form_tab_parent) {
		...
		if(this.options[Form.PROPERTY_FORM_TAB] == "true") {
			...
			//if(this.DOMform == null) {
				...
				var subTit = new Element('div',{id:this.id + '_ftmTitle'});
...
//				if(!divsInsteadOftable){
				if(typeof divsInsteadOfTable === 'undefined'){
					DOMtable = new Element("table", {
						id: "tbl" + this.id,
						title: this.frmTitle
					}).setStyle('table-layout', 'fixed');
				}else{
					DOMtable = new Element("div", {
						id: "tbl" + this.id,
						title: this.frmTitle,
						'class': "wasATable"
					}).setStyle('table-layout', 'fixed');
				}
				DOMtable.set('aria-labelledby',this.id + '_ftmTitle');
			
				var porcentaje = 100/cols;
				if(!MOBILE){
					var domColgroup = null
					if(typeof divsInsteadOfTable === 'undefined'){
						domColgroup = new Element("colgroup");
						for(j=0;j<cols;j++){
							var domCol = new Element("col");
							domCol.setStyle('width',porcentaje+'%');
							domCol.inject(domColgroup);
						}
						domColgroup.inject(DOMtable);
					}
				}
				if(typeof divsInsteadOfTable === 'undefined'){
					...
					var DOMtHead = new Element("div").addClass('wasTHead').setStyle('width',"100%");
					DOMtHead.inject(DOMtable);
					var DOMTrThs = new Element("div").addClass('wasTr').setStyle('width', porcentaje + "%");
					DOMTrThs.inject(DOMtHead);
					for(j=0;j<cols;j++){
						var domTh = new Element("div",{id:this.id +"_th_" + j, 'class': "wasTh"});
						domTh.inject(DOMTrThs);
				...
				if(typeof divsInsteadOfTable === 'undefined'){
					...
					var DOMtBody = new Element("div", {'class':"wasTBody"});
					DOMtBody.inject(DOMtable);
					for(var r = 0; r < rows; r++) {
						var tr = new Element('div',{'class': "wasTr"});
						for(var c = 0; c < cols; c++) {
...
//				}
			...
			//quitar rowspan si todos los elementos del tr tienen el mismo rowspan
			var trs = DOMtable.getElements('tr');
			for(x=0;x<trs.length;x++){  
				var tr = trs[x];
				if(tr.hasClass("selectableTR")) continue;
				...
				for(j=0;j<tds.length;j++){
					var td = tds[j];
					var tdRowspan = td.get('rowspan');
					if(td.firstChild==null || (td.firstChild!=null && td.firstChild.hasClass && !td.firstChild.hasClass("gridContainer"))){
						allGrids = false;	
					}
					if(first){
						lastRowspan=tdRowspan;
						first=false;
					}
					if(tdRowspan!=lastRowspan) {
						todosIguales=false;	
						break;
					}
				}
				if(todosIguales || allGrids){
					for(j=0;j<tds.length;j++){
						var td = tds[j];
						td.set('rowspan',1);
					}
...
//		}
	...
	parseField: function(xml) {
		...
		if (field.cols>=2){
			//Se calcula porcetaje necesario para alinear elementos ("justificar" - cada columna 95%)
			var perc = (100 * field.cols - 5)/ field.cols;  
			field.content.setStyle('width', perc.toFixed(2) + "%");
		}
