var Area = new Class({
	...
	apijs_setProperty: function(prp_name, prp_value) {
		...
		if(prp_name == Field.PROPERTY_NAME) {
			...
			if(textarea) {
				...
				var area_rows = this.rows? this.rows : "3";
	...
	parseXML: function() {
		...
		textarea.set('data-position',inputsCount++);
		...
		//TODO mejorar control de alto de textarea cuando no se tiene más elementos
		this.content.setStyle('min-height','75px');
		
		this.content.addClass('AJAXfield');
		this.content.getParent().addClass('field-textarea');
...
//		if(this.xml.getAttribute(Field.FUNC_CHANGE) && !window.editionMode) {
	...
	showFormListener: function() {
		...
		if(area)
			area.erase('data-disabledCheck');
	},
	
	hideFormListener: function() {
		var area = this.content.getElementsByTagName('textarea')[0];
		if(area)
			area.set('data-disabledCheck', 'true');
	},
