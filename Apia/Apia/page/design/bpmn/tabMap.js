function initTabMapa(){
    ...
    $('tabMap').addEvent("focus", function(evt){
		...
		//  call editor and show map
		// $('editor').contentWindow.map.loadXPDL($('txtMap').innerText);

		// Para que no vuelva a recargar y se pierdan los cambios diseñados IS_LOAD_MAP
			if (!IS_LOAD_MAP) {
				$('editor').src = CONTEXT + '/page/design/bpmn/svg/index.jsp';
				IS_LOAD_MAP = true;
			}
