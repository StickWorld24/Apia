...
var ApiaEventBinding = new Class({
	...
	initialize  : function(param_id, param_name, param_type, param_desc, att_type, value){
		...
		icons[0] = new Element('div.icon-item.search-icon')
			.addEvent('click', function(e){
				...
				if (property.name=="prpModal" || property.name=="prpEntity" || property.name=="prpDocType" ||
						...
						property.name=="entBinding" || property.name=="prpEditForm" || property.name=="prpQry" || property.name=="prpSelCusDocMon" ||
						property.name=="prpRootFolder" || property.name == 'prpDefaultFld'){
					...
					if(property.name == 'prpRootFolder' || property.name == 'prpDefaultFld') {
						openModal(TREE_STRUCT_MODAL, null, 30, 55, function(fldInfo){
							if(!fldInfo) return;
							var id = fldInfo[0], name = fldInfo[1];
...
function openModal(modalUrl, addParams, width, height, confirmCallback, closeCallback){
	...
	var mdlController = ModalController.openWinModal(url, width, height, undefined, undefined, false, false, false)
...
function loadEventProperties(xml, properties){
	...
	for (var j=0; j<events.length; j++){
		...
		for (var k=0; k<bindings.length; k++){
			...
			loadFromElementAttributes(bnd, b, ['param_id','param_name','param_type','param_desc','att_id','att_type','att_label','att_name']);
