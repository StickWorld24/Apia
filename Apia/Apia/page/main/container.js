	...
	*/
	...
	tabContainer.initTab = function(tab) {
		...
		if (tab.id == "tabHome") {
			...
			$('menuOptReport').addEvent('click', function() {
				
				var activeTab = $('subTabContainer').getElement('div.tab.active');
				if(activeTab) activeTab.getElement('span.remover').fireEvent('click');
				
				$('tab-1').set('src', 'apia.splash.SplashAction.run?langId=' + LANG_ID + '&dshId=10' + TAB_ID_REQUEST);
			...
			$('menuOptChangeEnv').addEvent('click', function(){				
				...
				var changeEnvOpt = this;
				$('menuOptContainer').getElements('li').each(function(ele){ 
					if (ele!=changeEnvOpt) ele.slide('toggle'); 
				});
				...
				if ($('collapseMenuEnvIcon').hasClass('menu-change-env-open')){
					var request = new Request({
						...
						onComplete: function(resText, resXml) {
							...
							var elements = resXml.getElementsByTagName('elements')[0];
							var children = elements.children? elements.children : elements.getChildren();  
							var userEnvs = children[4];
			...
			this.tabUser.userConfiguration.addEvent('click', function(evt){
                ...
                if((IS_SSO_LOGIN || AUTHENTICATION_LDAP == AUTHENTICATION_METHOD) && "admin" != USR_LOGIN){
                     var nodes = document.getElementById("btnPassConfModal").getElementsByTagName('*');
			...
			this.tabUser.userAboutApia.addEvent('click', function(evt){
				...
				showMessageObject(iframe, sbtAboutApia, "aboutApiaClass");
	...
	tabContainer.getTabByID = function(id) {
		var tabs = this.getElementsByClassName("tab");
		for(var i =0; i<tabs.length; i++){
			if(tabs[i].tabId && tabs[i].tabId == id){
				return tabs[i];
			}
		}
	};
	
	tabContainer.removeTabByID = function(id) {
		var tabs = this.getElementsByClassName("tab");
		for(var i =0; i<tabs.length; i++){
			if(tabs[i].tabId && tabs[i].tabId == id){
				tabContainer.removeTab(tabs[i]);
			}
		}
	};
...
//			openNewTab.inject(modal.footer);
	...
	tabContainer.addNewTab = function(title, url, fncId, evt, avoidTabControll, avoidAmountControll, avoidContentLoad, tokenId) {
		...
		$('tabLoadingImage').setStyle("display","block");
...
//			openNewTab.inject(modal.footer);
		...
		var iframeOpts = {
			...
			events: {
				load: function(e) {
					$('tabLoadingImage').setStyle("display","none");
