function initPage(){
	...
	var btnsReports = $$('*.reportBtn');
	for(var i = 0; i < btnsReports.length; i++) {
		btnsReports[i].addEvent('click', function(e) {
			e.stop();
			var tableData = $('tableData');
			...
			if (selectionCount(tableData) > 1) {
				...
				var idElems = e.target.getParent('div.reportBtn').get('id');
				var selectedRow = getSelectedRows($('tableData'))[0].getRowId();
				new Request({
					method: 'post',
					url: CONTEXT + URL_REQUEST_AJAX + '?action=checkReportDownload&repDwnInfo=' + idElems + '&rowId=' + selectedRow + TAB_ID_REQUEST,
					onRequest: function() { sp.show(true); },
					onComplete: function(resText, resXml) { modalProcessXml(resXml); sp.hide(true); }
				}).send(); 
//				
//				$('frmBtnReportInfo').set('value', e.target.getParent('div.reportBtn').get('id'));
//				$('frmBtnReportsId').set('value', );
//				doAjaxSubmit('frmBtnReports', true);
			}
		});
	}
...
function downloadReport(){
	SYS_PANELS.closeAll();
	createDownloadIFrame(LBL_DWN_REPORT, LBL_DOWNLOADING, "", "download", "", "", "", null);
}
