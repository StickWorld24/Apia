...
window.addEvent('load', function() {	
	...
	tipCapsLockPwd = FormCheck.makeTooltip('loginPassword', CAPS_TITLE);
	...
	$("loginUsr").addEvent("keyup" , function(evt){checkCapsLock(evt);});
	if($("loginPassword")){
		$("loginPassword").addEvent("keyup" , function(evt){checkCapsLock(evt);});
	}
	
	if("true" != SHOW_ENV_COMBO){
		var loginEnvironmentName = $("loginEnvironmentName");
		if (loginEnvironmentName) loginEnvironmentName.addEvent("keyup" , function(evt){checkCapsLock(evt);}); 
	} else {
...
function gotoSplash(){
	...
	if(referralUrl && referralUrl!=''&& referralUrl!='null'){
		location.href = referralUrl + '&fromFilter=true&tokenId=' + TOKEN_ID ;
		return;
	}
