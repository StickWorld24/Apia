...
<%@page import="java.util.Date"%><%@page import="com.st.util.ApiaTranslator"%><%@page import="com.dogma.EnvParameters"%><%@page import="com.dogma.UserData"%><%@page import="biz.statum.apia.web.bean.BasicBeanStatic"%><div class="tabContent" style="position:relative;"><div class="fieldGroup"><div class="title"><system:label show="text" label="sbtEjeDatEnt" /></div><div class="field required"><%
				if (! entityBean.isNewEntity()) {
					...
					%><span class="monitor-lbl" title="<system:label show="tooltip" label="lblEjeIdeEnt" />"><system:label show="text" label="lblEjeIdeEnt" />:<%if (BusEntityVo.IDENTIFIER_NUM_WRITE.equals(beVo.getBusEntIdeNum())){%>* <%}%></span><%
					if (BusEntityVo.IDENTIFIER_TXT_WRITE.equals(beVo.getBusEntIdePre())) {
						%><label style="display: none;" for="txtEntPre" title="<system:label show="tooltip" label="lblPrefix" />"><system:label show="text" label="lblPrefix" />:</label><input style="width: auto;" type="text" id="txtEntPre" name="txtEntPre" value="<%=BeanUtils.fmtStr(beInstVo.getBusEntInstNamePre())%>" size="5" maxlength="50"><%
					...
					if (BusEntityVo.IDENTIFIER_NUM_AUTO.equals(beVo.getBusEntIdeNum())) {
						...
						%><input title="<system:label show="tooltip" label="lblEjeIdeEnt" />" type="text" size=9 maxlength=9 name="txtEntNum" value="<%=BeanUtils.fmtInt(beInstVo.getBusEntInstNameNum())%>" <%if (BusEntityVo.IDENTIFIER_NUM_WRITE.equals(beVo.getBusEntIdeNum())){%> class="validate['required']" <%}%>><%
					}
				
					if (BusEntityVo.IDENTIFIER_TXT_WRITE.equals(beVo.getBusEntIdePos())) {
						out.print(Parameters.IDENTIFIER_SEPARATOR);
						%><label style="display: none;" for="txtEntPos" title="<system:label show="tooltip" label="lblSuffix" />"><system:label show="text" label="lblSuffix" />:</label><input style="width: auto;" id="txtEntPos" type="text" size=5 maxlength=50 name="txtEntPos" value="<%=BeanUtils.fmtStr(beInstVo.getBusEntInstNamePos())%>"  size="5" maxlength="50"><%
		...
		%><div class="field"><label class="monitor-lbl" for='createUser' title="<system:label show="tooltip" label="lblEjeUsuCreEnt" />"><system:label show="text" label="lblEjeUsuCreEnt" /><system:label show="separator" label=""/></label><span class='monitor-input'><%=createUser%><input type='text' id='createUser' style='display:none'></span></div><% 	 
		...
		%><div class="field"><label class="monitor-lbl" for='createDate' title="<system:label show="tooltip" label="lblEjeFecCreEnt" />"><system:label show="text" label="lblEjeFecCreEnt" /><system:label show="separator" label=""/></label><span class='monitor-input'><%=createDate%><input type='text' id='createDate' style='display:none'></span></div><div class="field"><%	
			...
			%><label for="busEntSta"  class="monitor-lbl" title="<system:label show="tooltip" label="lblEjeStaEnt" />"><system:label show="text" label="lblEjeStaEnt" /><system:label show="separator" label=""/></label><input type=hidden name="busEntSta" value="<%=BeanUtils.fmtInt(statusVo.getEntStaId())%>"><span class='monitor-input'><%=BeanUtils.fmtHTML(statusVo.getEntStaTitle())%></span><%} else { 
				...
				if (col != null) {%><label for="busEntSta" title="<system:label show="tooltip" label="lblEjeStaEnt" />"><system:label show="text" label="lblEjeStaEnt" /><system:label show="separator" label=""/></label><select id="busEntSta" name="busEntSta" <%Object obj = dBean; if ((request.getParameter("readOnly") != null) || (obj instanceof TaskBean) || (request.getAttribute("readOnly") != null)) {%> disabled="true" <%}%> ><option value="" selected><%	//Iterator it = col.iterator();
	   			...
	   			}%></div></div><div class="fieldGroup field-image-container"><div class="field"><label  for='hidEntImg' title="<system:label show="tooltip" label="prpImg" />" class="monitor-lbl"><system:label show="text" label="prpImg" />:&nbsp;</label><div class="img-container"><img id="busEntInstImage" alt='<system:label show="text" label="prpImg" />'/></div><input type='text' id='hidEntImg' style='display:none'></div></div></div>
