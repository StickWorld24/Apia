...
function addElementsOnChangeHighlight(elements, includeDisable) {
	...
	elements.each(function(ele) {
		...
		} else if (ele.hasClass('imagePicker')){
			ele.addEvent('change', fncSendFieldChangeTabHighlight.bind(ele, includeDisable));
			ele.set('initialValue', ele.style.backgroundImage.replace('url("','url("'+CONTEXT+'/'));
		}
	});
}
...
function initAdminFieldOnChangeHighlight(avoidInput, avoidSelect, avoidTextarea, element, includeDisable, avoidImagePicker) {
	...
	if (! avoidImagePicker) addElementsOnChangeHighlight(element.getElements('div.imagePicker'), includeDisable);
}
...
function fncSendFieldChangeTabHighlight(includeDisable) {
	...
	} else if (this.hasClass('imagePicker')){
		var isSameValue = this.style.backgroundImage == this.get('initialValue');
	}
	if (! isSameValue) {
