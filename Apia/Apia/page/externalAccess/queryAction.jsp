<%@page import="biz.statum.apia.web.LinkFactory"%><%@page import="com.dogma.controller.ThreadData"%><%@include file="../includes/startInc.jsp" %><%@page import="biz.statum.apia.web.bean.BasicBeanStatic"%><%@page import="com.apia.execution.ExternalService"%><%@page import="com.apia.core.CoreFacade"%><%@page import="com.dogma.vo.EnvironmentVo"%><%@page import="com.dogma.Parameters"%><%@page import="com.st.util.labels.LabelManager"%><%@page import="com.dogma.bean.DogmaAbstractBean"%><%@page import="com.dogma.bean.execution.ListTaskBean"%><%@page import="java.util.*"%><%@page import="com.dogma.UserData"%><%@page import="com.dogma.bean.DogmaAbstractBean"%><%@page import="com.st.util.StringUtil"%><%@page import="com.dogma.ExternalAccessLoader"%><%
	...
	if (userData == null){
		...
		BasicBeanStatic.saveUserDataStatic(request, userData);
	} 
	...
	if (request.getParameter("qryId") != null && !"null".equals(request.getParameter("qryId"))) {
		try{
			qryId = Integer.valueOf(request.getParameter("qryId"));
		}catch(NumberFormatException e){
			//Do nothing
		}
	}
	...
	//Obtenemos onlyGrid
	if (request.getParameter("onlyGrid") != null && !"null".equals(request.getParameter("onlyGrid"))) {
		onlyGrid = String.valueOf(request.getParameter("onlyGrid"));
	}
	
	//Obtenemos onlyChart
	if (request.getParameter("onlyChart") != null && !"null".equals(request.getParameter("onlyChart"))) {
		onlyChart = String.valueOf(request.getParameter("onlyChart"));
	}
...
// 	userData.setOnFinishURL(onFinishURL);
	...
	if (request.getParameter("query") != null && request.getParameter("query").length()>0 && !"null".equals(request.getParameter("query"))){
		qryId = Integer.valueOf(request.getParameter("query"));
	}
	
	String tokenId = userData.getTokenId();
	
	//http://localhost:8080/Apia/page/externalAccess/query.jsp?askLogin=Y&env=1&qryId=1004&onlyChart=true
	String redirectUrl=LinkFactory.craeteQueryActionInit(qryId, tokenId, env) + "&fromExternal=true&onlyChart="+request.getParameter("onlyChart")+"&onlyGrid="+request.getParameter("onlyGrid");
			
	if(filters!=null) redirectUrl += filters;
