...
function initDocuments(prefix,ajaxUploadStartAction,docTypePermitted, allowSign, dontAllowEdition) { //ej: docTypePermitted = {use: false, objId: '', objType: ''}
	...
	var useDocumentMonitor = toBoolean(GLOBAL_PARAM_ALLOW_REUSE_DOC_SV) || toBoolean(ENV_PARAM_ALLOW_REUSE_DOC_SV);
	var documentMonitorId = ENV_PARAM_DOC_MON_ID_SV;
				...
				showDocumentsModal(initDocumentMldPage, null,
						{
							...
							cusMonDoc: documentMonitorId,
							defMonDoc: useDocumentMonitor === true && documentMonitorId === null
						}, 
...
function showDocumentsModalByLang(langId, addTo, docLangGroup, extra) {
	...
	$(addTo).getParent('div.contentMenu-container').setStyle('display', 'none');
}
	...
	new Request({
		...
		onSuccess: function(resText, resXml) {
			...
			if (folderId){
		        ...
		        ITHit.WebDAV.Client.DavConstants.ProtocolTimeoutMs = 10000;
