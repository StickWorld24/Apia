...
//				});
				...
				if(field_xml && field_xml.nodeName == Grid.tags.gridField) {
					...
					switch (fieldType) {
						...
						case Field.TYPE_RADIO:
							field = new Radio(this.form, this.frmId, field_xml, row_xml);
							break;
					}
			...
			if(this.editionTable) {
				...
				var edit_button = new Element('div.editable-row', {tabindex:''}).inject(this.editionTable).addEvents({
					...
					click: function(e) {
						var modal = ModalController.openWinModal(CONTEXT + '/apia.execution.FormAction.run?action=editGrid&portalFormMode=' + portalFormMode + '&frmName=' + this_grid.options[Field.PROPERTY_GRID_FORM] + '&frmParent=' + this_grid.frmId.split('_')[0] + '&editionMode=true' + (this_grid.options[Field.PROPERTY_READONLY] ? '&ro=true' : '') + '&index=' + edition_index + TAB_ID_REQUEST, 640, 600, null, null, true, false, true);
			...
			if(this.editionTable) {
				...
				var newHeight = this.divBody.getHeight();
				if(this.divBody.scrollWidth >= this.divBody.getWidth()){
					newHeight = newHeight  - 15;//scroll horizontal
				}
...
//				grid_width -= 28;
		...
		tableHeader.setAttribute('aria-label', tableTitle);
		
		var divBody = new Element('div.gridBody').setStyles({
			width: 'auto',//grid_width,
		...
		tableBody.setAttribute('aria-label', tableTitle);
...
//			if(i == 0)
		...
		var divFooter = new Element('div.gridFooter').setStyle('width', 'auto');//grid_width);
...
//			});
		...
		var querySel = document.querySelectorAll('[id^=dropHandDiv]');
    	for (i = 0; i < querySel.length; i++) {
    		var newDiv = querySel[i]; 
    		if(newDiv.getElementsByClassName("docData").length>0 && newDiv.getElementsByClassName("docData")[0].innerText==""){
    			newDiv.setStyle('height', '17px');	
    		}
    		newDiv.setStyle('width', '152px');
    	}
	},
...
//		}
		...
		this.content.getElement('div.gridHeader').setStyle('width', 'auto');//grid_width);
		this.content.getElement('div.gridBody').setStyle('width', 'auto');//grid_width);
		this.content.getElement('div.gridFooter').setStyle('width', 'auto');//grid_width);
	},
...
var GridHeader = new Class({
	...
	initialize: function(th_container, title,tooltip, type, opts, fn_col_select, exec_ajax_call) {		
		...
		if(!tooltip || tooltip=='') tooltip = title;
		//Pasar a booleanos
