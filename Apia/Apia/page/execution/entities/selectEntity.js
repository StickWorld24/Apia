	...
	$('btnConf').addEvent("click", function(e) {
		...
		if(!$('processes').hasClass('hidden') && selPro.value!="" ){
			...
			var url = (busEntInstId == null) ? 
					'apia.execution.TaskAction.run?action=startCreationProcess&closeOnReturn=true&busEntId=' + cmbBusEntId.value + '&proId=' + selPro.value
				:
					'apia.execution.TaskAction.run?action=startAlterationProcess&closeOnReturn=true&busEntInstId=' + busEntInstId + '&busEntId=' + cmbBusEntId.value + '&proId=' + selPro.value;
