			...
			var aux = extra[i];
				if (aux!=null){
					...
					for (var k=0;k<aux.arr.length;k++){
						...
						if(auxRow.name == "chkShowAsInt") {
							domElement.addEvent('change', function() {
								filterChanger(this, 'input', 'chkShowAsIntFilter');
							});
						}
...
function filterChanger(element, filterType, filterName) {
	...
	var new_val = element.type.toLowerCase()=='checkbox' ? element.checked : element.get('value');
	...
	for(var m = 1; m < row.cells.length; m++) {
		...
		for( var n = 0; n < inps.length; n++) {
			if(inps[n].get('name') == filterName) {
				if (inps[n].type.toLowerCase() == 'checkbox')
					inps[n].checked = toBoolean(new_val);
				else
