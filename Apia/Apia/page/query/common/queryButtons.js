...
function initQueryButtons() {
	...
	if (clearFilters) {
		clearFilters.addEvent('click', function(evt){
			$$('input.queryFilter').each(function(ele){if(ele.hasAttribute("readonly")) return; ele.value = ""; ele.oldValue = ""; });
			$$('select.queryFilter').each(function(ele){if(ele.hasAttribute("readonly")) return; ele.selectedIndex = ""; ele.oldValue = ""; });
			suggestSearchButton();
