...
function callAdditionalProcessXmlInfoResponse() {
	...
	if (ajaxCallXml != null) {
		...
		for (var i = 0; i < cells.length; i++) {
			...
			var name = cell.getAttribute("name") + (cell.getAttribute("name").length > 0 ? ":" : "");
...
function callNavigateProcessXmlListResponse(){
	...
	if (ajaxCallXml != null) {
		...
		if (navigator) {
			if(totalRecords==0 && reachedMax){
				if (gridBody) {
					if (gridBody.noDataMessage) {gridBody.noDataMessage.dispose();}
					gridBody.noDataMessage = new Element('div', {'class': 'noDataMessage', html: GNR_TOT_RECORDS_REACHED}).inject(gridBody);
