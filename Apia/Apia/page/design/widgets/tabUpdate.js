function initUpdateTab(mode) {
	...
	// Acci�n al seleccionar Nodo especifico
	if ($('radExeSpecNode')) {
...
function clickOnRefreshRadio(clicked) {
	if (clicked == 2) {
		...
		$('cmbRefPeriod').set('value', PERIODICITY_EVERY_HOUR);
	} else {
...
function onSchExecChange(obj){
	if (obj.value == '0'){//Global
		$('exeNodeField').hide();
	} else {
		$('exeNodeField').show();
	}
