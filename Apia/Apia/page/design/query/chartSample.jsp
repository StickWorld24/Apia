...
try{
	...
	if (request.getParameter("chartType")!=null && request.getParameter("chtSubType") != null){
		...
		if (QryChartVo.PIE == Integer.parseInt(chartType)) { //Si es de tipo torta
			...
			if ("0".equals(chartSchema)){ //Si no se eligio un schema de colores para las series y se eligio un color personalizado
				
				if (QryChartVo.TYPEWATERFALL == Integer.parseInt(chartType)){
					WaterfallBarRenderer wFRenderer = (WaterfallBarRenderer) chart.getChart().getCategoryPlot().getRenderer();
					
					int valueR = Integer.parseInt(seriesColor.substring(0, 2), 16);
					int valueG = Integer.parseInt(seriesColor.substring(2, 4), 16);
					int valueB = Integer.parseInt(seriesColor.substring(4, 6), 16);
					
					wFRenderer.setFirstBarPaint(new Color(valueR,valueG,valueB));
					wFRenderer.setPositiveBarPaint(new Color(valueR,valueG,valueB));
					wFRenderer.setNegativeBarPaint(new Color(valueR,valueG,valueB));
					wFRenderer.setLastBarPaint(new Color(valueR,valueG,valueB));
			...
			//VER GRILLA
			if (QryChartVo.TYPELINES == Integer.parseInt(chartType)){
				CategoryPlot plot = chart.getChart().getCategoryPlot();	
				if ("true".equals(viewGrid)) {
