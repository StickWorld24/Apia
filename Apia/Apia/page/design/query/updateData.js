...
function initPage(){
			...
			if(colsName.length) {
				...
				showConfirm(msg, LBL_USE_AS_FILTER, function(result) {
					...
					if(result) {
						...
						//Para cada row, modificar 'Mostrar como', tooltip y mostrar como entero (si corresponde)
							...
							colsName.each(function(col_name) {
								for(var l = 0; l < tableBodyUserRows.length; l++) {
									if(tableBodyUserRows[l].cells[0].getElement('span').get('html') == col_name) {
										...
										var new_val = bodyRows[16].getElements('td')[cols[col_name]].getElement('input').checked;
										changeFilterColumn(tableBodyUserRows[l], 'input', 'chkShowAsIntFilter', new_val);
									}	
								}
							});
						}
	...
	if ($('tabActions')){
		$('tabActions').addEvent("focus", function(evt){chkActVieQry_click(); chkAddRpQry_click()});
//CAM_13379
...
function processBusClaParModalReturn(ret){
	...
	for (var j = 0; j < ret.length; j++){
		...
		for (var i=1;i<trows[0].cells.length&&addRet;i++) {
			if (trows[0].getElementsByTagName("TD")[i].getElement("SPAN").textContent.toUpperCase() == text.toUpperCase()) {
				addRet = false;
			}
		}
		if (addRet) {
