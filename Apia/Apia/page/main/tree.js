...
function processXmlCategoryTreePopulate(xml,elementSelect) {
	...
	if (xml != null && xml.getElementsByTagName("childs").length != 0) {
		...
		for (var i = 0; xmlRows != null && i < xmlRows.length; i++) {
			...
			var open 		= toBoolean(xmlRow.getAttribute("openInside"));
			...
			if (type == "L") {
				if (open) {
					htmlLi.addClass("notMenuFunctionality");
				} else {
					htmlLi.addClass("menuFunctionality");
				}
			} 
			if (childs != "0") {
			...
			htmlSpan.addEvent('click', function(evt) {
				...
				} else if (li.hasClass('notMenuFunctionality')) {
					var newWindow = window.open(li.urlToCall, "_blank");
					newWindow.focus();
				} else {
