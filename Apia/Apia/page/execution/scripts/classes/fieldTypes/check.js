var Check = new Class({
	...
	apijs_setFieldValue: function(value) {
		...
		if(typeOf(value) == "boolean") {
			...
			if(value.toUpperCase() == "TRUE" || value.contains('1'))
	...
	parseXML: function() {
		...
		if(this.form.readOnly) {
			...
			var span = new Element('span').addClass("asLabel").appendText(this.xml.getAttribute("attLabel"));
		...
		var span = new Element('span').addClass("asLabel");
		...
		check.set('data-position',inputsCount++);
	...
	parseXMLforGrid: function(td_container, grid_index, isGridReadonly) {
		...
		if(this.options[Field.PROPERTY_REQUIRED]) {
			...
			if(this.form.options[Form.PROPERTY_FORM_HIDDEN] == "true") {
				check.set('data-disabledCheck', 'true');
			}
		} else {
			check.setStyles({
				display: 'block',
