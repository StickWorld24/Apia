...
function initPage() {
    ...
    ['tabDefIde','attTab','envEvtTab','tabDoc','tabRel', 'biTab', 'tabBusEntPer'].each(function(ele) {
		ele = $(ele);
		if(ele){
...
function addNewLineTableAttBusEntRelation(attId, attName, entId, entName) {
	...
	registerValidation(htmlAttName);
	...
	registerValidation(htmlEntName);
	...
	if (form.childNodes.length > 0) {
		for (var i = 0; i < form.elements.length; i++) {
			...
			if (formElement.type == "select-multiple") {
				...
				if (formElement.getAttribute("data-hasdatepicker")=="true"){
...
function addRowAnc(table,extra){
	...
	if (!extra){
		setAutoCompleteBasic(elemToAutocomplete, null, 'searchBusinessEntities', true, CONTEXT + '/AutoCompleteAction.run?', true);
...
//	var busClaInputSearch = $("txtEntNameChild");
//	if (busClaInputSearch){
//	var entChilds = $
	if (!extra){
		setAutoCompleteBasic(nuevoElem, null, 'searchBusinessEntities', true, CONTEXT + '/AutoCompleteAction.run?', true);
		busClaInputSearch.addEvents({
			'optionNotSelected': function(evt) {
				btnAddBC.fade(0.4);
				btnAddBC.setAttribute('data-disabled', true);
...
function showMoreOptions(value) {
	if (value == 'F') {
		...
		$('radIdeNumSame').set('disabled','true');
	} else {
		$('moreEntOptions').style.display = 'none';
		$('radIdeNumSame').removeAttribute("disabled");
	}
}
...
function checkSelMonOpt() {
	if ($('showMonDoc').selectedIndex == 1) {
		$('selDocMon').removeClass('hidden');
	} else {
		$('selDocMon').addClass('hidden');
	}
}