...
function processXmlParameters(resXml){
	...
	if (parameters != null && parameters.length > 0 && parameters.item(0) != null) {
		...
		for(var i = 0; i < parameters.length; i++){
			...
			var div1 = new Element('div', {styles: {width: '100%', overflow: 'hidden', 'text-overflow':'ellipsis','white-space': 'pre', 'margin': '4px 0px'}}).inject(td1);
			if(isRequired)
				div1.set('text', '*');
			...
			var div2 = new Element('div', {styles: {width: '100%', overflow: 'hidden', 'text-overflow':'ellipsis','white-space': 'pre', 'margin': '4px 0px'}}).inject(td2);
			var spanType = new Element('span',{html: paramDesc }).inject(div2);
			
			//td3 TIPO
			var td3 = new Element("td", {styles: {width: '10%'}}).inject(tr);
			var div3 = new Element('div', {styles: {width: '100%', overflow: 'hidden', 'text-overflow':'ellipsis','white-space': 'pre', 'margin': '4px 0px'}}).inject(td3);
			var spanType = new Element('span',{html: getLblType(paramType)}).inject(div3);
			
			//td4 VALOR
			var td4 = new Element("td", {styles: {width: '30%'}}).inject(tr);
			var div4 = new Element('div', {styles: {width: '100%', overflow: 'hidden', 'text-overflow':'ellipsis','white-space': 'pre', 'margin': '4px 0px'}}).inject(td4);
			if (paramType == "D"){ //Date
