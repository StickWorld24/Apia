...
function initPage(){
	...
	initAdminActionsEdition(executeBeforeConfirm,true,false); //Acciones de los botones Confirmar, Volver, etc..  => Se comentan pues es necesario agregar los eventos de onConfirm desde este archivo
    ...
    if($('btnCustomConf')){
	    $('btnCustomConf').addEvent('click', function(e){
	    	if (e) e.stop();
	    	
	    	var schExec = $('cmbSchExec');
	    	var isGlobalExec = schExec && schExec.isVisible() && schExec.value==0;
	    	
	    	if (isGlobalExec && !IS_WIDGET_GLOBAL_SCH_ENABLE){
	    		showConfirm(LBL_WID_GLOB_EXEC_WARN, GNR_TIT_WARNING, 
						function(ret){					
								if (ret) $('btnConf').click();					
						}, 'modalWarning');
