...
function processXmlDocVersions(resXml){
	...
	if (information != null && information.length > 0 && information.item(0) != null) {
		...
		$('vdocExpDate').innerHTML = genData.getAttribute('docExpDate');
...
function downloadDocVersion(docVerId){
	...
	new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX + '?action=loadDownloadDocument&isAjax=true&id='+docId+'&version='+docVerId+ TAB_ID_REQUEST,
