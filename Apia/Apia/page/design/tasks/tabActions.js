...
function loadNotifications(){
	var request = new Request({
		...
		onComplete : function(resText, resXml) { 
			...
			showOnCloseMessage($(onTskCloseOpt));
		}
	}).send();
}
	...
	if (notifications != null && notifications.length > 0 && notifications.item(0) != null) {
		...
		if (msgs != null && msgs.length > 0 && msgs.item(0) != null) {
			...
			for(var i = 0; i < msgs.length; i++){
				...
				if (event=='M'){//onCloseMessage
					$('divMsgOnClose').getElement('textarea').value = msg.getAttribute("text");
				} else {
					var mEvent = traslateEvent(event);
...
function showOnCloseMessage(obj){
	var msgContainer = $('divMsgOnClose'); 
	var msgText = msgContainer.getElement('textarea');

	if (obj.selectedOptions[0].value == "1"){
		msgContainer.setStyle('display','');
		$('frmData').formChecker.register(msgText);		
	} else {
		msgContainer.setStyle('display','none');
		$('frmData').formChecker.dispose(msgText);
	}
}
